...
def forward_propagation(X, parameters):
    ...
    Z4 = tf.contrib.layers.fully_connected(P, 2, activation_fn=None)
...
def compute_cost(Z4, Y, parameters, regularization_beta):
    """
    Computes the cost
    ...
    cost = tf.reduce_mean(cost + regularization_beta * regularizer)

    return cost

def create_cnn_3conv_fc1_l2reg_model(n_H0,n_W0, n_C0, n_y):
    ...
    cost = compute_cost(Z4, Y, parameters, regularization_beta=0.0001)

    optimizer = tf.train.AdamOptimizer(learning_rate=0.006,beta1=0.9, beta2=0.999, epsilon=1e-08).minimize(cost)
